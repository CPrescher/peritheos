...
INDIRECT_DATA = {
    "fe088sio3_bridgmanite_ismailova_2016_300k_bm2": (
        "Supplementary Table S2 bundles only four selected crystallographic "
        "states, while Figure 3A plots substantially more compression and "
        "decompression markers for the published BM2 fit. The source gives no "
        "complete row list, inclusion/exclusion flags, regression objective, "
        "numerical weights, or row-wise Ne calibrant volumes. The four exact "
        "rows are checkpoint data, not a defensible reconstruction of the source "
        "fit input."
    ),
    ...
    for material_id in list_material_documents():
        ...
        for record in document["eos_records"]:
            ...
            base = {
                ...
                + list(record.get("implicit_parameters", ()))
