...
The campaign covers all **895** EOS records. **192** achieve uncertainty parity, **100** are numerically similar, **11** have a coupled source reconstruction without an independent EOS refit, **[63](#parity-not-achieved)** do not achieve parity, **[2](#composite-reconstructions)** are exact source-equation reconstructions, **1** have bounded partial refits, **526** cannot be directly refitted, and **0** attempts failed before comparison.
...
The following **163** sections cover every completed refit that does not meet the strict `parity` definition. `similar` means
...
**Source/data scope.** All nine 300 K and twelve high-temperature observations are bundled. Fit #2 additionally used 26 Fei et al. (2004) rows, absent from the Hirose article; their recovery and full-fit reproduction are handled separately.
...
- `gold_delta_archive_experimental_reference_bm3`: The archive supplies only a constructed coefficient table. Exact pre-correction inputs, a unified P-V dataset, pressure calibration, uncertainties, weights, and selection rules are not available, so an independent fit of the composite triplet is impossible. Property-level upstream recovery is recorded separately and includes row-level data only where the cited source actually prints it.
- `gold_hirose_2008_bm3_fit2`: All 21 Table 1 output pressures are reproduced, including 12 high-temperature observations. The full 38-row fit also used 26 Fei et al. (2004) observations not bundled by this initial Hirose audit. Their recovery and complete-fit reproduction are handled in the separate Fei follow-up.
