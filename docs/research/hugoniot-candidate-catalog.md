...
The complete repository inventory is in [`docs/data/hugoniot-material-inventory.csv`](../data/hugoniot-material-inventory.csv). It contains 286 material documents, 819 equilibrium records, and two production Hugoniot records, for 821 records total. Phase identity is taken from the explicit phase field where present and otherwise from the material name and crystal structure. Regenerate it with `python scripts/generate_hugoniot_material_inventory.py`.
...
- **Source:** Duffy and Ahrens, *J. Geophys. Res.* **100** B1, 529--542 (1995), [doi:10.1029/94JB02065](https://doi.org/10.1029/94JB02065). The relation, range, and uncertainty convention are printed in the abstract on p. 529 and as the MgO entry in Table 4 on p. 533; sample density is reported on p. 530 and individual observations in Table 3 on p. 533. An [author-hosted primary PDF](https://duffy.princeton.edu/sites/g/files/toruqf616/files/duffy_ahrens_seismo_1995.pdf) is publicly readable.
- **Provenance:** quoted coefficients plus an independent errors-in-variables reconstruction from all four Table 3 final shock states. Using the printed one-standard-deviation `Us` and `up` errors gives `c0=6.87044±0.10035 km/s` and `s=1.23799±0.04120`, reproducing the source rounding exactly. The paper does not publish its covariance or identify the residual variable, so this diagnostic covariance is not promoted to source-reported metadata.
- **Access and rights:** the author-hosted article is accessible, but the article states AGU copyright and no open data license was found. Peritheos does not redistribute the PDF, page images, figures, captions, or table typography. The four factual measurement rows are transcribed into a new machine-readable arrangement under a CC0 dedication limited to contributors' rights in that transcription and arrangement; it does not relicense the article or third-party material.
...
- **Recommendation:** implemented as `mgo_b1_duffy_ahrens_1995_hugoniot_5` with the four-row Table 3 dataset and a phase-restricted uncertainty-aware reproduction.
