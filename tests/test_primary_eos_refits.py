...
def test_primary_refit_ledger_covers_every_bundled_record_once():
    ...
    assert len(identifiers) == len(set(identifiers)) == 892
...
def test_primary_refit_summary_and_results_are_internally_consistent():
    ...
    assert ledger["summary"] == {"total": 892, **dict(sorted(statuses.items()))}
    assert statuses == {
        "bounded_partial": 1,
        "not_refittable": 525,
        "parity": 192,
...
def test_primary_refit_regression_examples_and_documentation_coverage():
    ...
    iridium = by_identifier["iridium_anzellini_2025_bm3_1"]
