...
def test_complete_material_library_is_bundled_and_valid():
    ...
    assert len(identifiers) == 223
    assert len(set(identifiers)) == 223
    assert sum(len(document["eos_records"]) for document in documents) == 600
    assert {
...
def test_migrated_records_have_completed_primary_source_audit():
    ...
    assert len(batch_identifiers) == 270
...
def test_migration_manifest_does_not_claim_a_dioptas_data_license():
    ...
    assert manifest["materials"] == 223
    assert manifest["eos_records"] == 600
