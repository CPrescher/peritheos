...
def test_complete_material_library_is_bundled_and_valid():
    ...
    assert sum(len(document["eos_records"]) for document in documents) == 821
...
def test_migrated_records_have_completed_primary_source_audit():
    ...
    assert len(batch_identifiers) == 418
...
def test_migration_manifest_does_not_claim_a_dioptas_data_license():
    ...
    assert manifest["eos_records"] == 821
    assert manifest["scientific_validation"]["audit_date"] == "2026-9-10"
