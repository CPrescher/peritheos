...
def test_complete_material_library_is_bundled_and_valid():
    ...
    assert len(identifiers) == 215
    assert len(set(identifiers)) == 215
    assert sum(len(document["eos_records"]) for document in documents) == 586
    assert {
...
def test_delta_project_benchmarks_are_not_in_the_production_catalog():
    documents = [
    ...
    assert not any("_lejaeghere_2016_" in record["identifier"] for record in records)
    assert not any(
        "_delta_archive_experimental_reference_" in record["identifier"]
        for record in records
    )
    assert not any(
        record.get("reference", {}).get("doi") == "10.24435/materialscloud:5e-mv"
        for record in records
    )


def test_sun_2010_benchmarks_are_not_in_the_production_catalog():
    records = [
        record
    ...
    assert not any(
        record.get("reference", {}).get("doi") == "10.1515/zna-2010-1-202"
        for record in records
    )
...
def test_migrated_records_have_completed_primary_source_audit():
    ...
    assert len(batch_identifiers) == 256
...
def test_primary_source_audit_report_covers_every_migrated_record():
    ...
    assert report["summary"] == {
        "records": 586,
        "primary_source_validated": 586,
    }
    assert report["audit_date"] == "2026-09-19"
...
def test_wang_2012_stishovite_vinet_mgd_record_and_table_transcription():
    ...
    assert source["thermal"]["model"] == ("asymptotic_power_law_mie_gruneisen_debye")
    ...
    assert record.volume(record.pressure(42.0, 1500.0), 1500.0) == pytest.approx(42.0)
...
def test_phase_egg_record_preserves_composition_structure_and_fit_scope():
    ...
    source = next(
        record
        for record in document["eos_records"]
        if record["identifier"] == "phase_egg_schulze_2018_bm3_1"
    )
    ...
    dataset = next(
        ...
        if dataset["identifier"] == "phase_egg_schulze_2018_table1_compression"
    )
...
def test_phase_egg_2019_lp_bm3_primary_source_record_and_supplement():
    document = get_material_document("phase_egg")
    assert "AlSiO3(OH) phase Egg" in document["aliases"]
    assert document["space_group"] == "P21/n"
    assert document["space_group_number"] == 14
    assert document["formula_units_per_cell"] == 4
    assert len(document["eos_records"]) == 3
    ...
    source = next(
        record
        for record in document["eos_records"]
        if record["identifier"] == "phase_egg_mookherjee_2019_bm3_lp_1"
    )
    ...
    dataset = next(
        dataset
        for dataset in document["datasets"]
        if dataset["identifier"] == "phase_egg_mookherjee_2019_supplement_lp_pv"
    )
    ...
    record = Material.from_eosmat(
        document, record_identifiers=["phase_egg_mookherjee_2019_bm3_lp_1"]
    ).eos_records[0]
...
def test_holmes_1989_platinum_equations_11_and_12_regression():
    ...
    pressure_300 = 798.31 * (1.0 - x) / x**2 * math.exp(7.2119 * (1.0 - x))
...
def test_no_doi_primary_sources_use_stable_article_or_report_locators():
    for material in ("coesite", "zircon"):
    ...
    lead = next(
        record
        for record in get_material_document("lead_fcc")["eos_records"]
        if record["identifier"] == "lead_fcc_fortes_2019_bm4_1"
    )
    assert lead["scientific_validation"]["primary_source_check"]["doi"] == (
        "10.5286/raltr.2019002"
    )
...
def test_migration_manifest_does_not_claim_a_dioptas_data_license():
    ...
    assert manifest["materials"] == 215
    assert manifest["eos_records"] == 586
    assert manifest["scientific_validation"]["audit_date"] == "2026-09-19"
