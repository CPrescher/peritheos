...
def test_complete_material_library_is_bundled_and_valid():
    ...
    assert len(identifiers) == 285
    assert len(set(identifiers)) == 285
    assert sum(len(document["eos_records"]) for document in documents) == 654
...
def test_migrated_records_have_completed_primary_source_audit():
    ...
    delta_identifiers = {
        identifier for identifier in native_identifiers if "_lejaeghere_" in identifier
    }
    batch_identifiers = (
        native_identifiers
        - legacy_native_identifiers
...
def test_migration_manifest_does_not_claim_a_dioptas_data_license():
    ...
    assert manifest["materials"] == 285
    assert manifest["eos_records"] == 654
