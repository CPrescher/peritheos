...
def test_complete_material_library_is_bundled_and_valid():
    ...
    assert len(identifiers) == 288
    assert len(set(identifiers)) == 288
    assert sum(len(document["eos_records"]) for document in documents) == 892
    assert {
...
def test_migrated_records_have_completed_primary_source_audit():
    ...
    delta_identifiers = {
        ...
        if "_lejaeghere_2016_" in identifier
    }
    batch_identifiers = (
        native_identifiers
        - legacy_native_identifiers
...
def test_migration_manifest_does_not_claim_a_dioptas_data_license():
    ...
    assert manifest["materials"] == 288
    assert manifest["eos_records"] == 892
